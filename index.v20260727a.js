...
// ============================================================
// 数据加载 - 按日期按需加载，避免一次性拉取4MB+数据
// ============================================================
const dateDataCache = {}; // 缓存已加载日期的数据 { date: { matches, predictions } }
...
// 加载指定日期的比赛+预测数据（按需，~50KB/天）
async function loadDateData(date) {
    if (dateDataCache[date]) {
        // 使用缓存
        state.allMatches = dateDataCache[date].matches;
        state.predictions = dateDataCache[date].predictions;
        renderMatches();
        return;
    }
    ...
    try {
        // 显示加载状态
        const footballEl = document.getElementById('view-football');
        const basketballEl = document.getElementById('view-basketball');
        if (footballEl) footballEl.innerHTML = '<div class="loading">加载中...</div>';
        if (basketballEl) basketballEl.innerHTML = '<div class="loading">加载中...</div>';
        
        const resp = await fetch(`/api/matches?date=${date}&include_predictions=true`);
        const dateMatches = await resp.json();
        ...
        dateMatches.forEach(m => {
            if (m.predictions && m.predictions.length > 0) {
                ...
                seen.forEach(p => {
                    predictions.push({ ...p, match_id: m.id });
                });
            }
        });
        console.log(`📅 ${date}: ${dateMatches.length}场比赛, ${predictions.length}条预测`);
        
        // 缓存
        dateDataCache[date] = { matches: dateMatches, predictions };
        
        // 更新全局状态
        state.allMatches = dateMatches;
        ...
        const footballEl = document.getElementById('view-football');
        if (footballEl) footballEl.innerHTML = '<div style="padding:20px;text-align:center;color:#ef4444;">数据加载失败</div>';
    }
}
// ============================================================
// 渲染函数
...
function renderDateTabs() {
    ...
    container.querySelectorAll('.date-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (newDate === state.currentDate) return;
...
function renderStats() {
    // 统计数字已从 /api/match-summary 加载，无需重新计算
    // allMatches 现在只包含当前日期的比赛
}
// ============================================================
// AI排行渲染 - 5维命中率表格
// ============================================================
