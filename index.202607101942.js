...
function renderMatchCard(match) {
    ...
    // 联赛名称 badge（足球绿色系，篮球橙色系）
    const leagueName = match.league_name || '';
    const leagueBadge = leagueName
        ? `<span style="display:inline-block;font-size:11px;line-height:1;padding:2px 6px;border-radius:4px;margin-right:6px;vertical-align:middle;flex-shrink:0;font-weight:600;${
            isBasketball
                ? 'background:rgba(249,115,22,0.25);color:#f97316;border:1px solid rgba(249,115,22,0.35);'
                : 'background:rgba(16,185,129,0.25);color:#10b981;border:1px solid rgba(16,185,129,0.35);'
          }">${esc(leagueName)}</span>`
        : '';
    ...
    return `
        <div class="view-item match-card-clickable" data-match-id="${match.id}">
            ${leagueBadge}<span class="time">${matchTime}</span>
            ${match.id ? `<span class="match-lottery-id">${match.id}</span>` : ''}
            <span class="teams">${esc(homeTeam)} ${match.status === '已确认' ? '<span class="score-inline">' + match.home_score + ':' + match.away_score + '</span>' : 'vs'} ${esc(awayTeam)}${handicapDisplay ? ' <span class="handicap-tag">' + handicapDisplay + '</span>' : ''}</span>
...
function computeDimStats(predictions, matchMap, activeAIs) {
    ...
    predictions.forEach(function(p) {
        ...
        const sport = m.sport_type || p.sport_type || "football";
