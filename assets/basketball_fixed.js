...
(async function loadBasketballPredictions() {
  ...
  try {
    ...
    const [predsResp, matchesResp] = await Promise.all([
      fetch('/api/predictions?sport=basketball'),
      fetch('/api/matches')
    ]);
    ...
    function renderMatchPredictions(teams, preds) {
      const predsHtml = preds.map(p => {
        ...
        // Check if prediction fields are all empty - use analysis as main display
        const hasPredictionData = (p.win_loss && p.win_loss !== '--') || 
                                  (p.handicap_win_loss && p.handicap_win_loss !== '--') ||
                                  (p.total_points) || (p.score_diff_range);

        // Build prediction summary
        const predSummary = hasPredictionData 
          ? `胜:${winLoss} 让:${handicap} ${totalPoints} ${scoreDiff}`
          : '详见分析';
        ...
        // Truncation: if analysisText is long, truncate and add expand/collapse
        const analysisId = 'analysis-' + (p.match_id || '') + '-' + aiName.replace(/[^a-zA-Z0-9]/g, '');
        const TRUNCATE_LEN = 200;
        let analysisDisplayHtml = '';
        if (analysisText) {
          const plainText = analysisText.replace(/<br>/g, '\n').replace(/<[^>]+>/g, '');
          if (plainText.length > TRUNCATE_LEN) {
            const shortText = analysisText.substring(0, TRUNCATE_LEN);
            analysisDisplayHtml = \`<div class="commentary-body text-gray-100 text-sm leading-relaxed">
              <p id="\${analysisId}-short">\${shortText}...</p>
              <p id="\${analysisId}-full" style="display:none">\${analysisText}</p>
              <a href="javascript:void(0)" id="\${analysisId}-btn" class="text-accent text-xs mt-1 inline-block"
                onclick="(function(el){var s=document.getElementById('\${analysisId}-short');var f=document.getElementById('\${analysisId}-full');if(f.style.display==='none'){f.style.display='';s.style.display='none';el.textContent='收起 ▲';}else{f.style.display='none';s.style.display='';el.textContent='展开全文 ▼';}})(this)">展开全文 ▼</a>
            </div>\`;
          } else {
            analysisDisplayHtml = \`<div class="commentary-body text-gray-100 text-sm leading-relaxed"><p>\${analysisText}</p></div>\`;
          }
        }
