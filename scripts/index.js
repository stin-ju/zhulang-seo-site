...
function renderMatchCard(match) {
    ...
    // 构建中间区域：让分始终在客队后，已完赛显示比分，未开赛显示VS
    let teamsHtml = '';
    const leagueName = match.league_name || match.league || '';
    const leagueStr = leagueName ? `${leagueName} ` : '';
    const awayWithHandicap = `${esc(awayTeam)}[${esc(handicap)}]`;
    if (isDone) {
        const hs = match.home_score != null ? match.home_score : '?';
        const as_ = match.away_score != null ? match.away_score : '?';
        teamsHtml += `<span class="match-teams">${esc(leagueStr)}${esc(homeTeam)} <span class="score-inline">${hs} : ${as_}</span> ${awayWithHandicap}</span>`;
    } else {
        teamsHtml += `<span class="match-teams">${esc(leagueStr)}${esc(homeTeam)} VS ${awayWithHandicap}</span>`;
    }
    ...
    if (isDone) {
        ...
        badges += `<span class="badge-sm status-pending">${matchTime || '待开赛'}</span>`;
    }
...
function renderPredictionsTable(predictions, match) {
    ...
    const rows = sorted.map(p => {
        ...
        if (isBasketball) {
            const winLoss = pred.win_loss || '-';
            const handicap = pred.handicap_win_loss || '-';
            const scoreDiff = pred.score_diff_range || '-';
            const totalPts = pred.total_points || '-';
            
            return `
            <tr>
                <td style="text-align:left;font-weight:500;color:#a78bfa;">${esc(p.ai_name || '')}</td>
...
function renderRanking() {
    ...
    container.innerHTML = '<table style="width:100%;font-size:13px;"><thead><tr style="color:#94a3b8;"><th style="text-align:left;padding:6px 4px;">排名</th><th style="text-align:left;padding:6px 4px;">AI</th><th style="text-align:right;padding:6px 4px;">胜平负</th><th style="text-align:right;padding:6px 4px;">让球</th><th style="text-align:right;padding:6px 4px;">大小球</th><th style="text-align:right;padding:6px 4px;">比分</th><th style="text-align:right;padding:6px 4px;">半全场</th></tr></thead><tbody>' + activeAIs.map((ai, i) => {
        const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : (i + 1);
        return '<tr><td style="padding:6px 4px;">' + medal + '</td><td style="padding:6px 4px;">' + (ai.ai_name || '') + '</td><td style="padding:6px 4px;text-align:right;">' + fmt(ai.hit_rate) + '</td><td style="padding:6px 4px;text-align:right;">' + fmt(ai.let_hit) + '</td><td style="padding:6px 4px;text-align:right;">' + fmt(ai.goals_hit) + '</td><td style="padding:6px 4px;text-align:right;">' + fmt(ai.score_hit) + '</td><td style="padding:6px 4px;text-align:right;">' + fmt(ai.half_full_hit) + '</td></tr>';
    }).join('') + '</tbody></table>';
}
// ============================================================
// AI Logo 渲染
