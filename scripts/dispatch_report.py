...
def generate_report():
    ...
    on_sale = query(conn, "SELECT count(*) as cnt FROM matches WHERE status = 'on_sale'")
    confirmed = query(conn, "SELECT count(*) as cnt FROM matches WHERE status = 'confirmed'")
    pending = query(conn, "SELECT count(*) as cnt FROM matches WHERE status = 'pending'")
    ...
    pending_matches = query(conn, """
        SELECT id, teams, match_time, sport_type, metadata->>'league' as league,
               EXTRACT(EPOCH FROM (NOW() - match_time))/3660 as hours_since_match
        FROM matches
    ...
    if pending_matches:
        ...
        for m in pending_matches:
            ...
            lines.append(f'| {m["id"]} | {league} | {m["teams"]} | {m["match_time"]} | {hours_str} |')

            # 异常检测: pending超过24小时
            if hours > 24:
                anomalies.append(f'⚠️ {m["id"]} ({m["teams"]}) pending 超过 {abs(hours):.0f} 小时')
        lines.append(f'')
    else:
    ...
    prediction_gaps = query(conn, """
        SELECT m.id, m.teams, m.sport_type, m.status,
               m.metadata->>'league' as league,
               COUNT(p.id) as pred_count,
               STRING_AGG(DISTINCT p.ai_name, ', ') as ai_list
        FROM matches m
        LEFT JOIN predictions p ON m.id = p.match_id
        WHERE m.status = 'on_sale' AND m.win_odds IS NOT NULL
        GROUP BY m.id, m.teams, m.sport_type, m.status, m.metadata
        HAVING COUNT(p.id) < 7
        ORDER BY COUNT(p.id) ASC, m.match_time ASC
    """)
    ...
    bb_on_sale = query(conn, "SELECT count(*) as cnt FROM matches WHERE sport_type = 'basketball' AND status = 'on_sale'")
    bb_pending = query(conn, "SELECT count(*) as cnt FROM matches WHERE sport_type = 'basketball' AND status = 'pending'")
    bb_pred = query(conn, """
        SELECT count(DISTINCT m.id) as cnt
        FROM matches m
        JOIN predictions p ON m.id = p.match_id
        WHERE m.sport_type = 'basketball' AND m.status = 'on_sale'
    """)
    ...
    if bb_pending[0]["cnt"] > 0:
        bb_pending_list = query(conn, """
            SELECT id, teams, match_time, EXTRACT(EPOCH FROM (NOW() - match_time))/3660 as hours
            FROM matches WHERE sport_type = 'basketball' AND status = 'pending'
            ORDER BY match_time ASC
        """)
        for m in bb_pending_list:
            hours = m.get('hours', 0) or 0
            if hours > 24:
                anomalies.append(f'⚠️ 篮球 {m["id"]} ({m["teams"]}) pending 超过 {abs(hours):.0f} 小时')
