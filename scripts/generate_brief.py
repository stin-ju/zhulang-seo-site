...
def fetch_matches_by_date(conn, date_str: str, sport_type: str = 'football') -> List[Dict]:
    ...
    with conn.cursor() as cur:
        cur.execute('''
            SELECT id, teams, match_time, status, selling_status, sport_type,
                   win_odds, draw_odds, lose_odds, handicap,
                   handicap_win_odds, handicap_draw_odds, handicap_lose_odds,
                   home_score, away_score, metadata
            FROM matches
            WHERE DATE(match_time) = %s AND sport_type = %s
            ORDER BY match_time
        ''', (date_str, sport_type))
...
def generate_commentary(matches: List[Dict], predictions: List[Dict], brief_type: str) -> str:
    ...
    for m in matches:
        ...
        match_summary.append({
            ...
            'odds': f"胜{m.get('win_odds') or '暂无'}/平{m.get('draw_odds') or '暂无'}/负{m.get('lose_odds') or '暂无'}",
            'predictions': [{'ai': p['ai_name'], 'spf': p.get('spf', ''), 'handicap': p.get('handicap_spf', '')} for p in preds]
        })
    
    # 构建prompt
    type_desc = "赛前预测简报" if brief_type == "prediction" else "赛后复盘简报"
    prompt = f"""你是一位专业的体育分析师，请为{type_desc}撰写一段精炼的总评。

今日比赛概况：
{json.dumps(match_summary, ensure_ascii=False, indent=2)}

要求：
1. 用专业但易懂的语言分析今日赛事整体情况
2. 指出AI预测的共识和分歧点
3. 分析赔率背后的市场态度
4. 给出1-2句"金句"作为总结
5. 字数控制在300-500字
...
def render_match_card(match: Dict, predictions: List[Dict]) -> str:
    ...
    # 赔率显示
    odds_html = ""
    if match.get('win_odds'):
        odds_html = f'''
        <div class="odds">
            <span>胜 {match.get('win_odds', '-')}</span>
            <span>平 {match.get('draw_odds', '-')}</span>
            <span>负 {match.get('lose_odds', '-')}</span>
        </div>'''
    ...
    <div class="match-card">
        <div class="match-header">
            ...
            <span class="match-time">{str(match.get('match_time', ''))[:16]}</span>
        </div>
