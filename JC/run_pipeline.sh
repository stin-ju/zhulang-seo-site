...
echo '【CT】传统彩赛程发现...'
if python3 CT/ct_discover.py >> $LOG_DIR/pipeline_$DATE.log 2>&1; then
    echo '  CT赛程发现完成'
else
...
if python3 CT/traditional_lottery_predict_v2.py --game 胜负彩 >> $LOG_DIR/pipeline_$DATE.log 2>&1; then
    echo '  CT胜负彩预测完成'
else
...
echo '【CT】传统彩自动结算...'
if python3 CT/ct_auto_settle.py >> $LOG_DIR/pipeline_$DATE.log 2>&1; then
    echo '  CT传统彩结算完成'
else
