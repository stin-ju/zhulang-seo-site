...
/usr/local/bin/python3 discover_matches.py >> $LOG_DIR/pipeline_$DATE.log 2>&1
echo '  完成'

# Step 2: AI预测（足球+篮球）
echo ''
echo '【Step 2/6】AI预测...'
/usr/local/bin/python3 auto_predict.py --sport football >> $LOG_DIR/pipeline_$DATE.log 2>&1 || echo '  足球预测完成(可能有警告)'
/usr/local/bin/python3 auto_predict.py --sport basketball >> $LOG_DIR/pipeline_$DATE.log 2>&1 || echo '  篮球预测完成(可能有警告)'
echo '  完成'

# Step 3: 数据质量检查（自动补全缺失的让球等）
echo ''
echo '【Step 3/6】数据质量检查与补救...'
/usr/local/bin/python3 data_quality_check.py >> $LOG_DIR/pipeline_$DATE.log 2>&1
echo '  完成'

# Step 4: 自动结算（补比分+结算预测）
echo ''
echo '【Step 4/6】自动结算...'
/usr/local/bin/python3 auto_settle.py >> $LOG_DIR/pipeline_$DATE.log 2>&1
echo '  完成'
...
/usr/local/bin/python3 generate_brief.py --date auto --type prediction --output both >> $LOG_DIR/pipeline_$DATE.log 2>&1 || echo '  简报生成失败(不阻断流水线)'
echo '  完成'
