...
# Prompt 模板
# ============================================================
FOOTBALL_PROMPT = """你是专业足球赛事分析师，请为以下欧冠资格赛生成竞彩预测。

比赛: {home_team} vs {away_team}
联赛: {league}
比赛时间: {match_time}
竞彩赔率: 胜{spf_win} 平{spf_draw} 负{spf_lose}
让球: {handicap}球 让胜{hdc_win} 让平{hdc_draw} 让负{hdc_lose}

请严格按以下JSON格式输出预测（不要输出其他内容）:
{{
  "spf": "胜或平或负",
  "handicap_spf": "让胜或让平或让负",
  "goals": 总进球数(0-7整数),
  "score": "比分如1-0",
  "half_full": "半全场(胜胜/胜平/胜负/平胜/平平/平负/负胜/负平/负负)",
  "confidence": 把握度(0.3-0.95),
  "analysis": "50字内分析"
}}"""

BASKETBALL_PROMPT = """你是专业篮球赛事分析师，请为以下美职女篮比赛生成竞彩预测。

比赛: {home_team} vs {away_team}
联赛: {league}
比赛时间: {match_time}
胜负赔率: 主胜{mnl_win} 客胜{mnl_lose}
让分: {handicap_line}分 让主胜{hdc_win} 让客胜{hdc_lose}
大小分: {hilo_line}分 大{hilo_over} 小{hilo_under}

请严格按以下JSON格式输出预测（不要输出其他内容）:
{{
  "win_loss": "主胜或客胜",
  "handicap_result": "让胜或让负",
  "total_points": "大或小",
  "score_diff": "分差区间(1-5/6-10/11-15/16-20/21-25/26+)",
  "confidence": 把握度(0.3-0.95),
  "analysis": "50字内分析"
}}"""
