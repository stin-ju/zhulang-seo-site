...
def _fetch_basketball_scores(date_str):
    """
    抓取篮球比分。titan007 的 nba_date.aspx 按美东时间分日期，
    北京时间早上的WNBA比赛在前一天的接口里。因此查目标日期+前一天，合并去重。
    编码用 gb18030 兼容繁体中文（gb2312 解不了繁体，会截断数据）。
    """
    ...
    for ds in dates_to_try:
        url = f"https://bf.titan007.com/nba_date.aspx?date={ds}&h=0&m=0&s=1"
        try:
            resp = requests.get(url, headers=headers, timeout=15)
...
def settle_basketball(row):
    ...
    # 让分=主队+handicap后比较，正数=主队受让，负数=主队让分
    # adjusted = home_score + handicap - away_score
    # adjusted > 0 = 让胜（主队赢盘），adjusted < 0 = 让负（客队赢盘）
    hc_pred = handicap_win_loss or handicap_result
    if hc_pred is not None and handicap is not None:
        try:
            hc_str = str(hc_pred).strip()
    ...
    # 4. score_diff_range 胜分差 - 支持多种格式
    if score_diff_range is not None:
        sdr = str(score_diff_range).strip()
        side = None
        winner = None
        low = high = None

        m1 = re.match(r'([主客])(\d+)-(\d+)(胜|负)', sdr)
        if m1:
            side = m1.group(1)
            low = int(m1.group(2))
            high = int(m1.group(3))
        ...
        if low is not None:
            if side is not None and winner is not None:
                is_hit = (side == actual_winner and winner == actual_win_str and low <= diff <= high)
            else:
    ...
    if half_win_loss is not None:
        ...
        if half_home is not None and half_away is not None:
            ...
            is_hit = (str(half_win_loss).strip() == actual)
