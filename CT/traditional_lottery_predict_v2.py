...
AI_CONFIGS = {
    ...
    "混元": {
        ...
        #   降级链：hy3(主) → plus(降1) → hy4-preview(降2) → pro(降3) → role-latest(降4) → role(降5) → ds-pro-202606(降6) → ds-pro-0813(降7) → ds-flash-vision(降8)
        "fallback_models": [
...
def get_existing_predictions(issue, game_type):
    """获取指定期号+类型已有的AI预测（从 predictions 表，ct_issue IS NOT NULL）

    由于 predictions 表是单场粒度（match_id=CT{issue}_{num}），同一 AI 会在同
    一期内有 13/14 条记录；这里通过 GROUP BY ai_name 去重，返回该期已产生过
    预测的 AI 集合（有任一场记录即视为已预测该期）。
    """
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
        """
        SELECT ai_name FROM predictions
        WHERE ct_issue IS NOT NULL
          AND ct_game_type = %s
          AND (match_id LIKE %s OR ct_issue = %s)
        GROUP BY ai_name
        """,
        (game_type, f'CT{issue}\\_%', f'CT{issue}')
    )
...
def save_predictions(issue, game_type, matches, ai_name, predictions):
    """保存预测到 predictions 表（v2：UPSERT on match_id, ai_name）

    predictions 表为单场粒度，每场一条记录：
      - match_id = CT{issue}_{场次号}（场次号形如 01、02...）
      - ct_issue = CT{issue}（如 CT26121）
      - ct_game_type = 玩法
      - ct_ren9 = 任9推荐场次（jsonb 数组，仅胜负彩）/ 若为比赛级别的任9场次号列表
      - prediction = 该场完整预测对象（jsonb）
      - spf_pred / score_pred / goals_pred / half_full_pred 从该场预测中提取
    """
    conn = get_db()
    cur = conn.cursor()
    ...
    match_by_num = {}
    for m in matches:
        match_by_num[str(m.get('num')).lstrip('0') or '0'] = m

    # predictions 数组里每条即一场；按场次号逐条 UPSERT
    # 若 predictions 为空（如某个 AI 解析失败），则不再写入任何行
    for item in predictions:
        num = str(item.get('match')).lstrip('0') or '0'
        match_id = f"{ct_issue}_{str(item.get('match')).zfill(2)}"
        ...
        if game_type in ('胜负彩', '任9'):
            spf_pred = item.get('spf')
            goals_pred = item.get('goals')
        elif game_type == '半全场':
            half_full_pred = item.get('bqc') or item.get('half_full')
        elif game_type == '进球彩':
            goals_pred = item.get('zjq') or item.get('goals')

        # 写入整场 prediction 对象（含场次号）
        pred_row = dict(item)
        pred_row['match_id'] = match_id
        pred_row['num'] = item.get('match')

        cur.execute("""
            INSERT INTO predictions (
                match_id, sport_type, ai_name, ct_issue, ct_ren9, ct_game_type,
                prediction, spf_pred, score_pred, goals_pred, half_full_pred,
                is_settled
            ) VALUES (
                %(match_id)s, 'traditional', %(ai_name)s, %(ct_issue)s, %(ct_ren9)s, %(ct_game_type)s,
                %(prediction)s::jsonb, %(spf_pred)s, %(score_pred)s, %(goals_pred)s, %(half_full_pred)s,
                FALSE
            )
        ...
        """, {
            ...
            'ai_name': ai_name,
            'ct_issue': ct_issue,
            'ct_ren9': json.dumps(ren9_picks, ensure_ascii=False) if ren9_picks is not None else None,
            'ct_game_type': game_type,
            'prediction': json.dumps(pred_row, ensure_ascii=False, default=str),
            'spf_pred': spf_pred,
            'score_pred': score_pred,
            'goals_pred': goals_pred,
            'half_full_pred': half_full_pred,
        })
