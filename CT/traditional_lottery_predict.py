        ...
        if last_brace > start:
            ...
            try:
                ...
                if isinstance(result, list) and len(result) > 0:
                    print(f" [解析修复] 截断JSON已修复，恢复{len(result)}场")
                    return result
            except:
                pass
...
def save_predictions(issue, game_type, matches, ai_name, predictions):
    ...
    # 生成任9推荐（仅胜负彩），同时保存为独立的任9记录
    ren9_json = None
    ren9_picks_data = None
    if game_type == '胜负彩':
        ren9_picks_data = generate_ren9(predictions)
        ren9_json = json.dumps(ren9_picks_data, ensure_ascii=False)
    ...
    # 胜负彩保存成功后，同步保存独立的任9记录
    if game_type == '胜负彩' and ren9_picks_data:
        ren9_matches_info = [mi for mi in matches_info if mi['num'] in [p.get('match','') for p in ren9_picks_data]]
        cur.execute(
            "SELECT id FROM traditional_predictions WHERE issue = %s AND game_type = %s AND ai_name = %s",
            (issue, '任9', ai_name)
        )
        if cur.fetchone():
            cur.execute("""
                ...
                SET predictions = %s::jsonb, matches_info = %s::jsonb, created_at = NOW()
                WHERE issue = %s AND game_type = %s AND ai_name = %s
            """, (json.dumps(ren9_picks_data, ensure_ascii=False),
                  json.dumps(ren9_matches_info, ensure_ascii=False),
                  issue, '任9', ai_name))
        else:
            cur.execute("""
                INSERT INTO traditional_predictions (game_type, ai_name, issue, predictions, matches_info, created_at)
            ...
            """, ('任9', ai_name, issue,
                  ...
                  json.dumps(ren9_matches_info, ensure_ascii=False)))
        conn.commit()
        print(f"  [{ai_name}] 任9已保存({len(ren9_picks_data)}场)")
        ...
        for ai_name in AI_NAMES:
            ...
            if raw:
                ...
                if parsed and len(parsed) >= len(matches):
                    ...
                    print(f"解析失败 (got {len(parsed) if parsed else 0} fields) raw前300字: {raw_preview}")
            else:
                print("无响应")
