...
function renderMatchCard(match) {
    ...
    if (handicapTag || bTotalHtml) {
        tagsHtml = `<div style="width:160px;flex-shrink:0;display:flex;justify-content:flex-end;align-items:center;gap:6px;">${handicapTag}${bTotalHtml}</div>`;
    }
...
function renderPredictionsTable(predictions, match) {
    ...
    const rows = sorted.map(p => {
        ...
        if (isBasketball) {
            const winLoss = (p.win_loss != null) ? p.win_loss : (pred.win_loss != null ? pred.win_loss : '-');
            const handicap = (p.handicap_win_loss != null) ? p.handicap_win_loss : (pred.handicap_win_loss != null ? pred.handicap_win_loss : '-');
            const scoreDiff = (p.score_diff_range != null) ? p.score_diff_range : (pred.score_diff_range != null ? pred.score_diff_range : '-');
            const totalPts = (p.total_points != null) ? p.total_points : (pred.total_points != null ? pred.total_points : '-');
