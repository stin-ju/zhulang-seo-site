...
(async function loadBasketballPredictions() {
  ...
  // Format score difference range
  function formatScoreDiff(scoreDiff) {
    if (!scoreDiff) return '胜分差:--';
    return `胜分差:${scoreDiff}`;
  }
  ...
  try {
    ...
    function renderMatchPredictions(teams, preds) {
      const predsHtml = preds.map(p => {
        ...
        const scoreDiff = formatScoreDiff(p.score_diff_range);
      ...
      return `
        <div class="rounded-lg bg-elevated/50 border border-border/50 p-4 mb-3">
...
<!-- 引入公共API模块 -->
<script type="module">
    import { fetchMatches, fetchPredictions, fetchAIStats, esc, fmtDate, fmtTime, showError, getCachedData, setCachedData } from './api.2026070805.js';
